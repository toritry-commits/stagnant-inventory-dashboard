-- ============================================================================
-- 固定資産台帳 月次簿価計算クエリ（SELECT版）- Lake Only Version
-- ============================================================================
-- 概要:
...
--
-- 変更点 (v2):
--   - finance.fixed_asset_register への依存を排除（CTEで計算）
--   - mart.stock_info への依存を排除（CTEで計算）
--   - すべての依存を lake.* のみに変更
--
...
-- ============================================================================
-- CTE 0: 償却資産税申告用期間（単一期間）
-- 2025年1月2日〜2026年1月1日
-- ============================================================================
WITH monthly_periods AS (
  SELECT
    DATE '2025-01-02' AS period_start,
...
lent_info AS (
  ...
  FROM `clas-analytics.lake.lent` AS lent
  INNER JOIN `clas-analytics.lake.lent_detail` AS lent_detail
    ON lent.id = lent_detail.lent_id
  WHERE lent.status IN ('Adjusting', 'Preparing', 'Lending', 'AdjustingReturn', 'PreparingReturn', 'Returned')
...
-- ============================================================================
-- CTE: B向け初回出荷日・リース開始日の計算 (lake.lent_to_b, lake.lent_to_b_detail から)
-- ============================================================================
to_b_info AS (
  SELECT
    stock_id,
  ...
  FROM `clas-analytics.lake.lent_to_b_detail` AS lent_to_b_detail
  INNER JOIN `clas-analytics.lake.lent_to_b` AS lent_to_b
    ON lent_to_b_detail.lent_to_b_id = lent_to_b.id
  WHERE lent_to_b.deleted_at IS NULL
    AND lent_to_b_detail.deleted_at IS NULL
  GROUP BY stock_id
),

-- ============================================================================
-- CTE: 売却案件名の取得 (lake.proposition, lake.contract, lake.lent_to_b から)
-- ============================================================================
sold_proposition AS (
  SELECT
    stock_id,
  ...
  INNER JOIN `clas-analytics.lake.lent_to_b` AS lent_to_b
    ...
    AND lent_to_b.lease_start_at IS NULL
  INNER JOIN `clas-analytics.lake.contract_destination` AS contract_destination
    ON contract_destination.id = lent_to_b.contract_destination_id
  INNER JOIN `clas-analytics.lake.contract` AS contract
    ON contract.id = contract_destination.contract_id
  INNER JOIN `clas-analytics.lake.proposition` AS proposition
    ON proposition.id = contract.proposition_id
  QUALIFY RANK() OVER(PARTITION BY stock_id ORDER BY lent_to_b.start_at DESC) = 1
),

-- ============================================================================
-- CTE: リース案件名の取得 (lake.proposition, lake.contract, lake.lent_to_b から)
-- ============================================================================
lease_proposition AS (
  SELECT
    lent_to_b_detail.stock_id,
    STRING_AGG(DISTINCT proposition.name) AS lease_proposition_name,
    MIN(lent_to_b.lease_start_at) AS lease_start_at
  FROM `clas-analytics.lake.proposition` AS proposition
  INNER JOIN `clas-analytics.lake.contract` AS contract
    ON proposition.id = contract.proposition_id
  INNER JOIN `clas-analytics.lake.contract_destination` AS contract_destination
    ON contract.id = contract_destination.contract_id
  INNER JOIN `clas-analytics.lake.lent_to_b` AS lent_to_b
    ON lent_to_b.contract_destination_id = contract_destination.id
    AND lent_to_b.deleted_at IS NULL
    AND lent_to_b.lease_start_at IS NOT NULL
  INNER JOIN `clas-analytics.lake.lent_to_b_detail` AS lent_to_b_detail
    ON lent_to_b_detail.lent_to_b_id = lent_to_b.id
    AND lent_to_b_detail.deleted_at IS NULL
  GROUP BY lent_to_b_detail.stock_id
),

-- ============================================================================
-- CTE 1: 基礎データ取得
-- 在庫・パーツ・サプライヤー・取得原価を結合（lake のみ）
-- ============================================================================
base_stock_data AS (
  SELECT
    s.id AS stock_id,
    ...
    CASE
      ...
      ELSE COALESCE(lent_info.first_shipped_at, to_b_info.first_shipped_at)
  ...
  LEFT JOIN lent_info ON s.id = lent_info.stock_id
...
with_costs_and_amort AS (
  SELECT
    *,
    -- 期首取得原価
    -- ※償却資産税用: 貸手リース中も所有権は当社にあるため、lease_start_at による判定を無効化
    CASE
      ...
      -- WHEN impossibled_at < period_start OR lease_start_at < period_start THEN 0  -- 会計用（貸手リースで資産除外）
      WHEN impossibled_at < period_start THEN 0  -- 償却資産税用（除売却のみで判定）
      ELSE actual_cost
    END AS acquisition_cost_opening,
    -- 増加取得原価（当月入庫またはリース再取得）
    -- ※償却資産税用: lease_start_at による判定を無効化
    CASE
      WHEN is_reveshare_flag THEN 0
      WHEN period_start <= lease_reacquisition_date AND lease_reacquisition_date <= period_end
           ...
           -- AND (lease_start_at IS NULL OR lease_start_at >= reacq_month)  -- 会計用（貸手リースで資産除外）
      THEN actual_cost
    ...
    -- 減少取得原価（当月除売却）
    -- ※償却資産税用: 貸手リース開始では減少せず、除売却時のみ減少
    CASE
      WHEN is_reveshare_flag THEN 0
      -- WHEN impossibled_at < period_start OR lease_start_at < period_start THEN 0  -- 会計用
      -- WHEN impossibled_at < reacq_month OR lease_start_at < reacq_month THEN 0  -- 会計用
      -- WHEN lease_start_at > period_end THEN 0  -- 会計用
      -- WHEN lease_start_at IS NULL AND (impossibled_at IS NULL OR impossibled_at > period_end) THEN 0  -- 会計用
      WHEN impossibled_at < period_start THEN 0  -- 償却資産税用
      WHEN impossibled_at < reacq_month THEN 0  -- 償却資産税用
      WHEN impossibled_at IS NULL OR impossibled_at > period_end THEN 0  -- 償却資産税用
      ELSE actual_cost
    ...
    CASE
      ...
      -- WHEN inspected_at > period_end OR impossibled_at <= period_end OR lease_start_at <= period_end THEN 0  -- 会計用
      WHEN inspected_at > period_end OR impossibled_at <= period_end THEN 0  -- 償却資産税用（除売却のみで判定）
      ELSE actual_cost
...
SELECT
  ...
  inspected_at,
  -- first_shipped_at,  -- 償却資産税用（減価償却計算用のため不要）
  -- impairment_date,  -- 償却資産税用（減価償却計算用のため不要）
  impossibled_at,
  convert_impossibility_to_ja(classification_of_impossibility) AS impossibility_classification_ja,
  sold_proposition_name,
  -- lease_start_at,  -- 償却資産税用（貸手リース関連は不要）
  -- lease_proposition_name,  -- 償却資産税用（貸手リース関連は不要）
  lease_reacquisition_date,
  -- monthly_depreciation,  -- 償却資産税用（減価償却計算用のため不要）
  -- 期首（取得原価のみ）(Opening Balance)
  -- ※償却資産税用: 減価償却累計額・減損累計額・簿価は不要のためコメントアウト
  acquisition_cost_opening,
  -- accum_depr_opening,  -- 会計用
  -- impairment_opening,  -- 会計用
  -- calc_book_value(acquisition_cost_opening, accum_depr_opening, impairment_opening, is_reveshare_flag) AS book_value_opening,  -- 会計用
  -- 増加（取得原価のみ）(Increase)
  acquisition_cost_increase,
  -- accum_depr_increase,  -- 会計用
  -- impairment_increase,  -- 会計用
  -- calc_book_value(acquisition_cost_increase, accum_depr_increase, impairment_increase, is_reveshare_flag) AS book_value_increase,  -- 会計用
  -- 減少（取得原価のみ）(Decrease)
  acquisition_cost_decrease,
  -- accum_depr_decrease,  -- 会計用
  -- impairment_decrease,  -- 会計用
  -- calc_book_value(acquisition_cost_decrease, accum_depr_decrease, impairment_decrease, is_reveshare_flag) AS book_value_decrease,  -- 会計用
  -- 期中（償却資産税では不要）(Interim)
  -- interim_depr_expense,  -- 会計用
  -- impairment_increase AS interim_impairment,  -- 会計用
  -- 期末（取得原価のみ）(Closing Balance)
  acquisition_cost_closing
  -- accum_depr_closing,  -- 会計用
  -- impairment_closing,  -- 会計用
  -- calc_book_value(acquisition_cost_closing, accum_depr_closing, impairment_closing, is_reveshare_flag) AS book_value_closing,  -- 会計用
  -- 累計値（償却資産税では不要）(Cumulative - for flexible period calculation)
  -- cumulative_acquisition_cost_increase,  -- 会計用
  -- cumulative_acquisition_cost_decrease,  -- 会計用
  -- cumulative_accum_depr_increase,  -- 会計用
  -- cumulative_accum_depr_decrease,  -- 会計用
  -- cumulative_impairment_increase,  -- 会計用
  -- cumulative_impairment_decrease,  -- 会計用
  -- cumulative_depr_expense,  -- 会計用
  -- cumulative_impairment  -- 会計用

FROM with_cumulative
-- 償却資産税用: 以下の条件で絞り込み
--   1. 取得原価が20万円を超える資産のみ対象
--   2. パーツ名に「棚卸対象」を含むものは除外
--   3. 資産分類が「賃貸用固定資産」のもののみ対象
WHERE actual_cost > 200000
  AND part_name NOT LIKE '%棚卸対象%'
  AND classify_asset(supplier_name, sample) = '賃貸用固定資産'
