-- ============================================================
-- 滞留在庫ダッシュボード: 余りパーツ (term別縦持ち)
-- ============================================================
-- 作成日: 2026-01-15
-- 基盤: stagnant_inventory_dashboard_v4.sql
--
-- 出力形式: SKU情報 + term別の余りパーツ数・簿価 (縦持ち)
-- 用途: 余りパーツの減損リスク分析、活用検討
--
-- 出力カラム:
...
-- SKU×パーツ×エリア別の余りパーツ (term別)
sku_part_leftover_by_term AS (
  SELECT
    spd.sku_hash,
    spd.part_id,
    ls.business_area,
    ...
    SUM(ls.book_value) AS leftover_book_value
  FROM leftover_stocks ls
  INNER JOIN sku_part_detail spd ON ls.part_id = spd.part_id
  GROUP BY spd.sku_hash, spd.part_id, ls.business_area, ls.category
),
...
sku_leftover_summary AS (
  SELECT
    ...
    SUM(CASE WHEN term = 'impaired' THEN leftover_cnt ELSE 0 END) AS leftover_impaired,
    SUM(CASE WHEN term = 'term1' THEN leftover_cnt ELSE 0 END) AS leftover_term1,
    SUM(CASE WHEN term = 'term2' THEN leftover_cnt ELSE 0 END) AS leftover_term2,
    SUM(CASE WHEN term = 'term3' THEN leftover_cnt ELSE 0 END) AS leftover_term3,
    SUM(CASE WHEN term = 'term4' THEN leftover_cnt ELSE 0 END) AS leftover_term4,
    SUM(leftover_book_value) AS total_book_value,
    SUM(CASE WHEN term = 'impaired' THEN leftover_book_value ELSE 0 END) AS impaired_book_value,
    SUM(CASE WHEN term = 'term1' THEN leftover_book_value ELSE 0 END) AS term1_book_value,
    SUM(CASE WHEN term = 'term2' THEN leftover_book_value ELSE 0 END) AS term2_book_value,
    SUM(CASE WHEN term = 'term3' THEN leftover_book_value ELSE 0 END) AS term3_book_value,
    SUM(CASE WHEN term = 'term4' THEN leftover_book_value ELSE 0 END) AS term4_book_value
  FROM sku_part_leftover_by_term
  GROUP BY sku_hash
),
...
sku_horizontal AS (
  SELECT
    ...
    qe.term3_end,
    qe.term4_end,
    COALESCE(sls.total_leftover, 0) AS total_leftover_cnt,
    COALESCE(sls.leftover_impaired, 0) AS impaired_leftover_cnt,
    COALESCE(sls.leftover_term1, 0) AS term1_leftover_cnt,
    COALESCE(sls.leftover_term2, 0) AS term2_leftover_cnt,
    COALESCE(sls.leftover_term3, 0) AS term3_leftover_cnt,
    COALESCE(sls.leftover_term4, 0) AS term4_leftover_cnt,
    COALESCE(sls.total_book_value, 0) AS total_book_value,
    COALESCE(sls.impaired_book_value, 0) AS impaired_book_value,
    COALESCE(sls.term1_book_value, 0) AS term1_book_value,
    COALESCE(sls.term2_book_value, 0) AS term2_book_value,
    COALESCE(sls.term3_book_value, 0) AS term3_book_value,
    COALESCE(sls.term4_book_value, 0) AS term4_book_value,
...
final_output AS (
  ...
  FROM sku_horizontal
  CROSS JOIN UNNEST([
    STRUCT('impaired' AS term, CAST(NULL AS DATE) AS term_end_date, impaired_leftover_cnt AS leftover_cnt, impaired_book_value AS book_value),
    STRUCT('term1', term1_end, term1_leftover_cnt, term1_book_value),
    STRUCT('term2', term2_end, term2_leftover_cnt, term2_book_value),
    STRUCT('term3', term3_end, term3_leftover_cnt, term3_book_value),
    STRUCT('term4', term4_end, term4_leftover_cnt, term4_book_value)
  ])
  WHERE total_leftover_cnt > 0
)
