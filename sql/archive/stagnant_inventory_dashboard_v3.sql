...
-- 変更履歴:
--   [2025-12-29] v3
--   - 簿価・簿価累計カラム（52-59）の集計ロジックを「在庫単位」に変更
--     → v2では組み上げ商品の簿価を「商品カテゴリ」で集計していたが、
--        v3では在庫ごとの減損カテゴリ（sac.category）で集計する
--     → これにより、商品に含まれる在庫が異なる期に減損対象となる場合、
--        それぞれの期に正しく簿価が計上される
--   - 簿価平均単価カラム（60-63）は変更なし（商品カテゴリベースのまま）
--
--   [2025-12-26]
--   - 取得原価を簿価に変更（clas-analytics.mart.monthly_stock_valuationから取得）
--   - サプライヤー取得ロジック修正: deleted_atフィルタをサブクエリ内に移動
--     → 最大バージョンが論理削除されている場合でも正しいサプライヤーを取得可能に
--   - business_area IS NULLの在庫（法人直送倉庫）を全エリアのカウントから除外
--     → 以前は関東/関西/九州すべてに重複カウントされていた
--   - 簿価平均単価カラム追加（商品のみの簿価 ÷ 商品数）
--   - 在庫ID表示に●マーク追加（その期の減損対象を識別）
--   - 在庫ID表示形式を[パーツID:倉庫:〇点]に変更
--   - ロケーション除外条件を削除（全ロケーションを対象に変更）
--     → 除外していた17ロケーション（ピック済、廃棄申請待ち等）も集計対象に
--
--   [2025-12-23] 初版作成
...
SELECT
  ...
  -- 17. 期末1_減損予定_在庫id_商品
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term1_assembled_ids, '') END AS `期末1_減損予定_在庫id_商品`,

  -- 21. 期末2_減損予定_在庫id_商品
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term2_assembled_ids, '') END AS `期末2_減損予定_在庫id_商品`,

  -- 22. 期末3_減損予定_在庫id_商品
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term3_assembled_ids, '') END AS `期末3_減損予定_在庫id_商品`,

  -- 23. 期末4以降_減損予定_在庫id_商品
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term4_assembled_ids, '') END AS `期末4以降_減損予定_在庫id_商品`,

  -- 24. 減損済_在庫id_商品
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.impaired_assembled_ids, '') END AS `減損済_在庫id_商品`,

  -- 25. 期末1_減損予定_在庫id_余りパーツ
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term1_leftover_ids, '') END AS `期末1_減損予定_在庫id_余りパーツ`,

  -- 26. 期末2_減損予定_在庫id_余りパーツ
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term2_leftover_ids, '') END AS `期末2_減損予定_在庫id_余りパーツ`,

  -- 27. 期末3_減損予定_在庫id_余りパーツ
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term3_leftover_ids, '') END AS `期末3_減損予定_在庫id_余りパーツ`,

  -- 28. 期末4以降_減損予定_在庫id_余りパーツ
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.term4_leftover_ids, '') END AS `期末4以降_減損予定_在庫id_余りパーツ`,

  -- 29. 減損済_在庫id_余りパーツ
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(stock_ids.impaired_leftover_ids, '') END AS `減損済_在庫id_余りパーツ`,

  -- 30. 期末1_減損予定_商品数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sis.term1_product_cnt, 0) END AS `期末1_減損予定_商品数`,

  -- 31. 期末2_減損予定_商品数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sis.term2_product_cnt, 0) END AS `期末2_減損予定_商品数`,

  -- 32. 期末3_減損予定_商品数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sis.term3_product_cnt, 0) END AS `期末3_減損予定_商品数`,

  -- 33. 期末4以降_減損予定_商品数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sis.term4_product_cnt, 0) END AS `期末4以降_減損予定_商品数`,
  ...
  -- 36. 期末1_減損予定_余りパーツ数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sls.leftover_term1_total, 0) END AS `期末1_減損予定_余りパーツ数`,

  -- 37. 期末2_減損予定_余りパーツ数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sls.leftover_term2_total, 0) END AS `期末2_減損予定_余りパーツ数`,

  -- 38. 期末3_減損予定_余りパーツ数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sls.leftover_term3_total, 0) END AS `期末3_減損予定_余りパーツ数`,

  -- 39. 期末4以降_減損予定_余りパーツ数
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sls.leftover_term4_total, 0) END AS `期末4以降_減損予定_余りパーツ数`,
  ...
  -- 48. 期末1_減損予定_商品+余りパーツ累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    IFNULL(sis.term1_product_cnt, 0) + IFNULL(sls.leftover_term1_total, 0)
  END AS `期末1_減損予定_商品+余りパーツ累計`,

  -- 49. 期末2_減損予定_商品+余りパーツ累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    (IFNULL(sis.term1_product_cnt, 0) + IFNULL(sls.leftover_term1_total, 0)) +
    (IFNULL(sis.term2_product_cnt, 0) + IFNULL(sls.leftover_term2_total, 0))
  END AS `期末2_減損予定_商品+余りパーツ累計`,

  -- 50. 期末3_減損予定_商品+余りパーツ累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    (IFNULL(sis.term1_product_cnt, 0) + IFNULL(sls.leftover_term1_total, 0)) +
    (IFNULL(sis.term2_product_cnt, 0) + IFNULL(sls.leftover_term2_total, 0)) +
    (IFNULL(sis.term3_product_cnt, 0) + IFNULL(sls.leftover_term3_total, 0))
  END AS `期末3_減損予定_商品+余りパーツ累計`,

  -- 51. 期末4以降_減損予定_商品+余りパーツ累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    (IFNULL(sis.term1_product_cnt, 0) + IFNULL(sls.leftover_term1_total, 0)) +
    (IFNULL(sis.term2_product_cnt, 0) + IFNULL(sls.leftover_term2_total, 0)) +
    (IFNULL(sis.term3_product_cnt, 0) + IFNULL(sls.leftover_term3_total, 0)) +
    (IFNULL(sis.term4_product_cnt, 0) + IFNULL(sls.leftover_term4_total, 0))
  END AS `期末4以降_減損予定_商品+余りパーツ累計`,

  -- 52. 期末1_減損予定_簿価
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sbv.term1_book_value, 0) END AS `期末1_減損予定_簿価`,

  -- 53. 期末2_減損予定_簿価
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sbv.term2_book_value, 0) END AS `期末2_減損予定_簿価`,

  -- 54. 期末3_減損予定_簿価
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sbv.term3_book_value, 0) END AS `期末3_減損予定_簿価`,

  -- 55. 期末4以降_減損予定_簿価
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE IFNULL(sbv.term4_book_value, 0) END AS `期末4以降_減損予定_簿価`,

  -- 56. 期末1_減損予定_簿価_累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    IFNULL(sbv.term1_book_value, 0)
  END AS `期末1_減損予定_簿価_累計`,

  -- 57. 期末2_減損予定_簿価_累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    IFNULL(sbv.term1_book_value, 0) + IFNULL(sbv.term2_book_value, 0)
  END AS `期末2_減損予定_簿価_累計`,

  -- 58. 期末3_減損予定_簿価_累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    IFNULL(sbv.term1_book_value, 0) + IFNULL(sbv.term2_book_value, 0) + IFNULL(sbv.term3_book_value, 0)
  END AS `期末3_減損予定_簿価_累計`,

  -- 59. 期末4以降_減損予定_簿価_累計
  CASE WHEN spi.sku_id IS NULL THEN NULL ELSE
    IFNULL(sbv.term1_book_value, 0) + IFNULL(sbv.term2_book_value, 0) + IFNULL(sbv.term3_book_value, 0) + IFNULL(sbv.term4_book_value, 0)
  END AS `期末4以降_減損予定_簿価_累計`,

  -- 60. 期末1_減損予定_商品_簿価平均単価（分母0の場合は0を返す）
  CASE
    WHEN spi.sku_id IS NULL THEN NULL
    WHEN IFNULL(sis.term1_product_cnt, 0) = 0 THEN 0
    ELSE ROUND(IFNULL(sbv.term1_product_book_value, 0) / sis.term1_product_cnt, 0)
  END AS `期末1_減損予定_商品_簿価平均単価`,

  -- 61. 期末2_減損予定_商品_簿価平均単価（分母0の場合は0を返す）
  CASE
    WHEN spi.sku_id IS NULL THEN NULL
    WHEN IFNULL(sis.term2_product_cnt, 0) = 0 THEN 0
    ELSE ROUND(IFNULL(sbv.term2_product_book_value, 0) / sis.term2_product_cnt, 0)
  END AS `期末2_減損予定_商品_簿価平均単価`,

  -- 62. 期末3_減損予定_商品_簿価平均単価（分母0の場合は0を返す）
  CASE
    WHEN spi.sku_id IS NULL THEN NULL
    WHEN IFNULL(sis.term3_product_cnt, 0) = 0 THEN 0
    ELSE ROUND(IFNULL(sbv.term3_product_book_value, 0) / sis.term3_product_cnt, 0)
  END AS `期末3_減損予定_商品_簿価平均単価`,

  -- 63. 期末4以降_減損予定_商品_簿価平均単価（分母0の場合は0を返す）
  CASE
    WHEN spi.sku_id IS NULL THEN NULL
    WHEN IFNULL(sis.term4_product_cnt, 0) = 0 THEN 0
    ELSE ROUND(IFNULL(sbv.term4_product_book_value, 0) / sis.term4_product_cnt, 0)
  END AS `期末4以降_減損予定_商品_簿価平均単価`,
  ...
  -- 53-56. 期末日（参照用）
  qe.term1_end AS `期末1_日付`,
  qe.term2_end AS `期末2_日付`,
  qe.term3_end AS `期末3_日付`,
  qe.term4_end AS `期末4_日付`
