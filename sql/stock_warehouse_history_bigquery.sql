-- ============================================================================
-- 在庫の倉庫履歴取得クエリ（BigQuery版）
-- ============================================================================
-- 概要:
--   指定時点での在庫の所在倉庫を取得するクエリ
--   基幹システム用クエリをBigQuery用に変換
--
-- 用途:
--   償却資産税申告用に以下の情報を取得
...
StockSnapshot AS (
    SELECT
        ...
        CASE
            ...
            WHEN s.impossibled_at <= DATE '2026-01-01' THEN s.impossibled_at
            ELSE NULL
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            FROM `clas-analytics.lake.stock_history`
...
LastWarehouseHistory AS (
    ...
    FROM (
        ...
        FROM
            `clas-analytics.lake.stock_history` h
        INNER JOIN
            `clas-analytics.lake.location` l ON h.location_id = l.id
        WHERE
...
SELECT
    ...
    ss.restored_deleted_at AS deleted_at_jan1,
...
FROM
    StockSnapshot ss
    LEFT JOIN `clas-analytics.lake.location` l_curr ON ss.restored_location_id = l_curr.id
    LEFT JOIN LastWarehouseHistory lwh ON ss.stock_id = lwh.stock_id
    LEFT JOIN `clas-analytics.lake.location` l_last ON lwh.location_id = l_last.id
;
