-- ============================================================================
-- 余りパーツ「解消提案」レポート 自己完結版 (引継ぎ用 / 他テーブル非依存)
-- ============================================================================
-- 目的 : 余りパーツが「なぜ余っているか (どの相方パーツが不足か)」と
--        「いくら投資すれば商品に変えられるか」を SKU 軸で出す。
-- 出力 : 1行 = SKU x エリア (相方パーツ・余り在庫ID・案件はセル内改行で集約)
-- 依存 : clas-analytics.lake.* のみ。mart / sandbox 等の中間テーブル不要。
--        (通常運用版は leftover_reason_report.sql + leftover_reason_report_by_sku.sql
--         の2段構成で sandbox にテーブルを作る。本ファイルはその統合版)
-- 仕様 : - セット数上限 = 余りパーツ(貸出可能)を使い切る商品数の最大値
--        - 単価 = 相方パーツの在庫cost平均 -> 発注明細cost平均 のフォールバック
--        - 必要金額 = 単価 x 追加必要点数 (相方に余りがあれば差し引く)
--        - 案件 = 現役 (Preparing/InUse) の lent_to_b 経由のみ。終了済みは '-'
...
-- 由来 : 余り計算は sp_sku_inventory_v2.sql 丸写し (leftover_id_count_standalone.sql と同一)
-- ============================================================================
...
-- ============================================================================
-- Final statement: 詳細 (SKU x エリア x 相方パーツ) を TEMP テーブルに作成
-- ============================================================================
CREATE TEMP TABLE leftover_reason_detail AS
WITH
...
ORDER BY
  CASE b.`エリア` WHEN '関東' THEN 1 WHEN '関西' THEN 2 ELSE 3 END,
  b.`必要金額合計` DESC NULLS LAST,
  b.`SKU_ID`;
